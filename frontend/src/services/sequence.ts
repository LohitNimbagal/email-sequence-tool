...
export const fetchSequences = async (): Promise<Sequence[]> => {
    try {
        const response = await fetch(`${import.meta.env.VITE_SERVER_BASE_URI}/api/sequences`, {
            method: 'GET',
            headers: {
...
export const fetchSequence = async (sequenceId: string): Promise<Sequence> => {
    try {
        const response = await fetch(`${import.meta.env.VITE_SERVER_BASE_URI}/api/sequences/${sequenceId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            },
